...
export class MapGenerator {
  ...
  private checkIntersection(r: Room, newRoom: Room) {
    return r.x1 <= newRoom.x2 && r.x2 >= newRoom.x1 && r.y1 <= newRoom.y2 && r.y2 >= newRoom.y1;
  }
  ...
  private genTunnels(n: number): void {
    ...
    for (let i = Math.min(firstCenter[0], secondCenter[0]); i < Math.max(firstCenter[0], secondCenter[0]) + 1; i++) {
      this.maps[i][secondCenter[1]] = '0';
    }
    for (let i = Math.min(firstCenter[1], secondCenter[1]); i < Math.max(firstCenter[1], secondCenter[1]) + 1; i++) {
