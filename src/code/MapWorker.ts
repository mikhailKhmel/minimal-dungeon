...
export class MapWorker {
  ...
  private mobMover(mapData: Array<IEntity>) {
    ...
    function attackPlayer(mapData: Array<IEntity>, mob: IMob, player: IPlayer) {
      ...
      const x2 = mob.x + 1;
      const y2 = mob.y + 1;
      if (player.x >= x1 && player.y >= y1 && player.x <= x2 && player.y <= y2) {
    ...
    function chooseDirection(mob: IMob, player: IPlayer): { x: number; y: number } {
      ...
      if (player.x >= x1 && player.y >= y1 && player.x <= x2 && player.y <= y2) {
        ...
        futurePlaces.push({
          ...
          place: mapData.find((x) => x.x === mob.x + DIRECTIONS.UP.x && x.y === mob.y + DIRECTIONS.UP.y) ?? null,
        });
        futurePlaces.push({
          direction: DIRECTIONS.LEFT,
          place: mapData.find((x) => x.x === mob.x + DIRECTIONS.LEFT.x && x.y === mob.y + DIRECTIONS.LEFT.y) ?? null,
        });
        futurePlaces.push({
          direction: DIRECTIONS.RIGHT,
          place: mapData.find((x) => x.x === mob.x + DIRECTIONS.RIGHT.x && x.y === mob.y + DIRECTIONS.RIGHT.y) ?? null,
        });
        futurePlaces.push({
          direction: DIRECTIONS.DOWN,
          place: mapData.find((x) => x.x === mob.x + DIRECTIONS.DOWN.x && x.y === mob.y + DIRECTIONS.DOWN.y) ?? null,
        });
        futurePlaces = futurePlaces.filter((x) => x.place && x.place.type === EntityType.Void);

        for (const futurePlace of futurePlaces) {
          const newDiffX = Math.abs(player.x - (mob.x + futurePlace.direction.x));
          const newDiffY = Math.abs(player.y - (mob.y + futurePlace.direction.y));
        ...
        const futurePlace = mapData.find((data) => data.x === mob.x + x && data.y === mob.y + y) ?? null;
    ...
    if (entity.type === EntityType.Player) {
      switch (direction) {
        case DIRECTIONS.DOWN: {
          renderType =
            entity.renderType === RenderType.PlayerDown1
              ? RenderType.PlayerDown2
              : entity.renderType === RenderType.PlayerDown2
              ? RenderType.PlayerDown3
              : entity.renderType === RenderType.PlayerDown3
              ? RenderType.PlayerDown1
              : RenderType.PlayerDown1;
          break;
        }
        case DIRECTIONS.UP: {
          renderType =
            entity.renderType === RenderType.PlayerUp1
              ? RenderType.PlayerUp2
              : entity.renderType === RenderType.PlayerUp2
              ? RenderType.PlayerUp3
              : entity.renderType === RenderType.PlayerUp3
              ? RenderType.PlayerUp1
              : RenderType.PlayerUp1;
          break;
        }
        case DIRECTIONS.LEFT: {
          renderType =
            entity.renderType === RenderType.PlayerLeft1
              ? RenderType.PlayerLeft2
              : entity.renderType === RenderType.PlayerLeft2
              ? RenderType.PlayerLeft3
              : entity.renderType === RenderType.PlayerLeft3
              ? RenderType.PlayerLeft1
              : RenderType.PlayerLeft1;
          break;
        }
        case DIRECTIONS.RIGHT: {
          renderType =
            entity.renderType === RenderType.PlayerRight1
              ? RenderType.PlayerRight2
              : entity.renderType === RenderType.PlayerRight2
              ? RenderType.PlayerRight3
              : entity.renderType === RenderType.PlayerRight3
              ? RenderType.PlayerRight1
              : RenderType.PlayerRight1;
          break;
        }
        default:
      ...
      if (direction.x === 0 && direction.y === 1) {
        renderType =
          entity.renderType === RenderType.MobDown1
            ? RenderType.MobDown2
            : entity.renderType === RenderType.MobDown2
            ? RenderType.MobDown3
            : entity.renderType === RenderType.MobDown3
            ? RenderType.MobDown1
            : RenderType.MobDown1;
      } else if (direction.x === 0 && direction.y === -1) {
        renderType =
          entity.renderType === RenderType.MobUp1
            ? RenderType.MobUp2
            : entity.renderType === RenderType.MobUp2
            ? RenderType.MobUp3
            : entity.renderType === RenderType.MobUp3
            ? RenderType.MobUp1
            : RenderType.MobUp1;
      } else if (direction.x === -1 && direction.y === 0) {
        renderType =
          entity.renderType === RenderType.MobLeft1
            ? RenderType.MobLeft2
            : entity.renderType === RenderType.MobLeft2
            ? RenderType.MobLeft3
            : entity.renderType === RenderType.MobLeft3
            ? RenderType.MobLeft1
            : RenderType.MobLeft1;
      } else if (direction.x === 1 && direction.y === 0) {
        renderType =
          entity.renderType === RenderType.MobRight1
            ? RenderType.MobRight2
            : entity.renderType === RenderType.MobRight2
            ? RenderType.MobRight3
            : entity.renderType === RenderType.MobRight3
            ? RenderType.MobRight1
            : RenderType.MobRight1;
      } else {
      ...
      if (direction.x === 0 && direction.y === 1) {
        renderType =
          entity.renderType === RenderType.BossDown1
            ? RenderType.BossDown2
            : entity.renderType === RenderType.BossDown2
            ? RenderType.BossDown3
            : entity.renderType === RenderType.BossDown3
            ? RenderType.BossDown1
            : RenderType.BossDown1;
      } else if (direction.x === 0 && direction.y === -1) {
        renderType =
          entity.renderType === RenderType.BossUp1
            ? RenderType.BossUp2
            : entity.renderType === RenderType.BossUp2
            ? RenderType.BossUp3
            : entity.renderType === RenderType.BossUp3
            ? RenderType.BossUp1
            : RenderType.BossUp1;
      } else if (direction.x === -1 && direction.y === 0) {
        renderType =
          entity.renderType === RenderType.BossLeft1
            ? RenderType.BossLeft2
            : entity.renderType === RenderType.BossLeft2
            ? RenderType.BossLeft3
            : entity.renderType === RenderType.BossLeft3
            ? RenderType.BossLeft1
            : RenderType.BossLeft1;
      } else if (direction.x === 1 && direction.y === 0) {
        renderType =
          entity.renderType === RenderType.BossRight1
            ? RenderType.BossRight2
            : entity.renderType === RenderType.BossRight2
            ? RenderType.BossRight3
            : entity.renderType === RenderType.BossRight3
            ? RenderType.BossRight1
            : RenderType.BossRight1;
      } else {
  ...
  private entityMove(mapData: Array<IEntity>, direction: { x: number; y: number }, entityId: string) {
    ...
    const futurePlaceIndex = mapData.findIndex((data) => data.x === futureX && data.y === futureY);
  ...
  private playerAttack(mapData: Array<IEntity>, player: IPlayer) {
    const leftItem = mapData.find((data) => data.x === player.x - 1 && data.y === player.y) ?? null;
    const upItem = mapData.find((data) => data.x === player.x && data.y === player.y - 1) ?? null;
    const rightItem = mapData.find((data) => data.x === player.x + 1 && data.y === player.y) ?? null;
    const downItem = mapData.find((data) => data.x === player.x && data.y === player.y + 1) ?? null;
    ...
    if (attackedMob) {
      ...
      mapData[mapData.findIndex((x) => x.id === attackedMob?.id)].renderType =
        attackedMob.type === EntityType.Boss ? RenderType.RedBoss : RenderType.RedMob;
    } else if (openedChest) {
    ...
    for (const redBoss of redBosses) {
      if (redBoss.data.hp <= 0) {
        ...
        if ((mapData[mapData.findIndex((x) => x.type === EntityType.Player)] as IPlayer).data.inventory.length < 9) {
          (mapData[mapData.findIndex((x) => x.type === EntityType.Player)] as IPlayer).data.inventory.push(
            ItemCreator.create(ItemType.Scroll),
          );
        }
      } else {
